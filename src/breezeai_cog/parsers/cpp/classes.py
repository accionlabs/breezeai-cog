            ...
            if spec is not None:  # a nested type definition (class/struct/union/enum)
                if spec.type == "enum_specifier":
                    enum_cls, enum_stmts = build_enum(
                        spec, source, path, parent_id=cid, seen_ids=seen_ids,
                ...
                if function_declarator_of(member.child_by_field_name("declarator")) is None:
                    # A member variable / constant, not a function. C++ class fields are
                    # otherwise captured nowhere, so emit each as a flat statement (its
                    # `text` — incl. any `= value` initializer — is queryable).
                    if capture and member.type == "field_declaration":
                        statements.append(
                            member_statement(
...
def build_enum(
    ...
    *,
    ...
    statements = (
        emit_enum_members(
            body, source, path,
            member_types={"enumerator"}, parent_id=cid, limit=limit, seen_ids=seen_ids,
        )
        if capture
        else []
    )
